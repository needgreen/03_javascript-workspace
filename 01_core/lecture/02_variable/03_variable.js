...
// 1. var (es5까지만 사용한 편, 최근 사용 안하는 추세)

var userId;
console.log(userId);      // 특징1 : 할당 전 undefined(초기화 없이 선언 가능)
var userId = 'user01';    // 특징2 : 재선언 - 가능 (동일 스코프(범위)에서 중복 선언 가능)
...
function varScopeTest() {
  ...
  console.log(userPwd);
  console.log(userAge);   // 특징4 : scope - 함수 스코프 (선언된 위치가 속해있는 블럭에서 사용 가능)
}
...
//let userName = '홍길동';    // 특징2 : 재선언 - 불가능(동일한 범위(scope) 내 중복 선언 불가능)
...
function letScopeTest() {
  let userAddr = '서울';     // 값을 초기화 하는 것, 값 설정

  if(true) {
    let userAge = '10';      // 선언
  }
  console.log(userAddr);
  //console.log(userAge);    // 특징4 : scope - 블럭스코프 (선언된 블럭 내에서만 사용 가능) 
}
letScopeTest();


// 3. const - 상수를 선언하기 위해 사용되는 키워드 (상수:항상 동일한 수, 변경이 불가능한 값)
//const userHobby;             // 특징1 : 할당전 - 사용 불가능(초기화를 해야 선언 가능)
const userHobby = '요리';
...
//특징4 : scope - 블럭 스코프 (선언된 블럭 내에서만 사용가능, let과 동일한 특징)
...
console.log(y);                   // 선언과 동시에 초기화 할 경우 선언문만 분리해서 우선 처리함
var y = 20;    // var y;  y = 20;  => 선언과 동시에 초기화 한 구문
console.log(y);
...
// 2) 함수 호이스팅 : 함수(function)도 범위 내에서 최상위로 끌어올려져 1순위로 호출될 수 있음
